...
import { Company } from '../../components/common/companyTable';

interface Body {
  search?: string;
  page?: number;
  is_company_and_new: boolean;
  is_new?: boolean;
...
interface GetCompaniesAPIResponse extends AxiosResponse {
  data: {
    data: Company[];
    pagination: {
      count: number;
...
const Dashboard: React.FC<DashboardProps> = ({ profile }) => {
  ...
  const getCachedCompaniesFromQueryData = (queryKey: string | any[]) => {
    const res = queryClient.getQueryData<GetCompaniesAPIResponse>(queryKey);
    let newCompanies = {
      items: [] as Company[],
      totalItemCount: 0,
      totalPages: 0,
  ...
  const [state, setState] = useStateCallback({
    ...
    sortMethod: 'both' as 'both' | 'ASC' | 'DESC',
  ...
  const onSearchCompanies = (e: React.FormEvent<HTMLButtonElement>) => {
    e.preventDefault();
    const newCompanies = getCachedCompaniesFromQueryData([
      'companies',
      1,
  ...
  const onSelectHeaderCheckbox = () => {
    if (isHeaderCheckBoxSelected) {
      ...
      let newSelectedCompanies = [] as number[];
  ...
  const onDropdownValueChange = (selected: { value: string; key: string }) => {
                    ...
                    <Dropdown
                      ...
                      onClick={(option) =>
                        onDropdownValueChange(
                          option as { key: string; value: string },
                        )
                      }
